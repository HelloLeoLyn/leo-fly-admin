    ...
    <el-form :model="dataBody" ref="dataBody" :rules="rules" label-width="180px" :inline="false" size="normal">
      <template v-for="layoutName in  schema.hierarchy.structure[this.schema.hierarchy.root]">
        ...
        <el-form-item :key="layoutName" v-else :label="schema.data[layoutName].fields.label">
          <el-button type="primary" size="default" @click="saveAsTemplate">存为模板</el-button>
          <el-form v-if="schema.hierarchy.structure[layoutName]" :model="dataBody" :ref="layoutName" :rules="rules"
            label-width="80px" :inline="false" size="normal">
            <template v-for="(layoutName2, index) in schema.hierarchy.structure[layoutName]">
              <!-- <el-form-item :label="layoutName2" :key="index" v-if="layoutName2=='saleProp'">
                {{ schema.data[layoutName2].fields }}
              </el-form-item> -->
              <el-form-item :label="schema.data[layoutName2].fields.label" :key="index" v-if="schema.data[layoutName2]"
                ...
                <!-- {{ handleObj(schema.data[layoutName2]).fields }} -->
                {{ schema.data[layoutName2].fields }}
              </el-form-item>
              <!-- <el-form-item :label="layoutName2" :key="index" v-else>
                {{ layoutName2 }}===========================
              </el-form-item> -->
            </template>
            <el-form-item>
