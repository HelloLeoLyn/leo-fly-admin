  ...
  <div class="app-container leo-container">
    <div id="menu">
        ...
        <el-col :lg="16" :md="16" :sm="12">
          <el-form label-width="120px">
            <el-form-item label="分类">
              ...
              <Category1688 v-if="product.categoryId" :value="product.categoryId"
                categoryContent="设置为产品名" @change="e => (product.categoryId = e)" />
            </el-form-item>
            ...
            <div v-for="(value, key) in product_more.base" :key="key">
              <el-form-item :label="jsonFilterKeys[key].label"
      ...
      <div class="filter-container">
        <leo-product-reset :id="$route.params.id" :key="$route.params.id" v-waves
          class="filter-item" type="primary" content="清除" />
        <LeoWebCollector :key="imageCollectorKey" type="primary" iconClass="" content="采集数据" v-waves
          class="filter-item" style="margin-left: 20px" @onConfirm="handleLeoWebCollectorConfirm"
          :values="{
            code: '',
            id: id,
            codes: product.code,
        ...
        <el-button @click="handleCollectClick" v-waves class="filter-item" type="primary"
          style="margin-left: 20px">采集图片
        </el-button>
        <el-button @click="onConfirmProductSave" v-waves class="filter-item" type="primary"
          style="margin-left: 20px">保存
        </el-button>
        <el-button v-waves class="filter-item" type="primary" style="margin-left: 20px"
          @click="showHistory()">销售记录
        </el-button>
        <el-button @click="done" v-waves class="filter-item" type="primary"
          style="margin-left: 20px">完成
        </el-button>
      </div>
      <h2>图片</h2>
      <ImageAddCover v-if="product.code" :images="images" :product-id="$route.params.id"
        :code="product.code[0]">
      </ImageAddCover>
      <h2>{{ $t('autoPart.oe') }}</h2>
      ...
      <leo-array-string v-if="product.refNo" :span="8" actionType="out" :list="product.refNo"
        ref="refNo" :key="leoArrayAtringRefNo" />
      <h2>{{ $t('autoPart.carfitment') }}</h2>
      <el-tabs v-model="modelYearName" type="card" tab-position="top"
        ...
        <el-tab-pane label="详情" name="all">
          <el-table v-if="product.model" :data="JSON.parse(product.model)">
            ...
            <el-table-column :label="$t('autoPart.displacement')"
              prop="Displacement"></el-table-column>
        ...
        <el-tab-pane label="Model&Year" name="modelYear">
          <el-button type="danger" v-for="(text, key) in selectedArray" :key="key"
            icon="el-icon-delete" @click="selectedArray.splice(key, 1)">{{ text }}</el-button>
    ...
    <LeoHistory :productId="history.productId" v-model="history" :key="history.count"
      direction="btt"></LeoHistory>
  </div>
</template>
<script>
